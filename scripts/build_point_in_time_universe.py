...
def _snapshot_rows(
    ...
    *,
    ...
    for quote in quotes:
        if not _is_equity_ticker(quote.ticker):
            continue
    ...
    for decision in decisions:
        ...
        for item in candidates:
            issuer = str(item["issuer_name"]) or str(item["ticker"])[:4]
        ...
        if len(selected) < top_n:
            raise ValueError(
                f"{decision}: only {len(selected)} point-in-time candidates satisfy the rules."
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build a survivorship-safe weekly universe from full B3 COTAHIST "
    ...
    for quote in standard_quotes:
        ...
        if ticker not in union:
            continue
        issuer_by_ticker[ticker] = quote.issuer_name.strip().upper() or ticker[:4]
        if quote.isin:
    ...
    manifest = {
        ...
        "bias_disclosure": (
            ...
            "its effective_date. Delisted and renamed symbols remain eligible while "
            "they exist in the historical full-market archive; no continuity filter "
            "to future years is applied."
        ),
