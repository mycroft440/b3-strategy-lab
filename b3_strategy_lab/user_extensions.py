...
# Register the indicator-driven catalog through the same extension API used by
# user strategies. This keeps strategies.py generic and makes every new engine
# immediately available to sweep_strategies() and portfolio_strategies().
for _item in _INDICATOR_STRATEGIES:
    _register_strategy(
        _item.name,
        family=_item.family,
