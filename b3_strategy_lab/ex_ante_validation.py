...
def latest_complete_calendar_year(
    ...
    *,
    ...
    The current calendar year is never eligible. A candidate year must contain
    sessions in January and December and enough observations to reject truncated
    datasets. This is intentionally conservative and fail-closed.
    """
    ...
    if not complete:
        raise ValidationError("No complete historical calendar year is available for holdout.")
    return max(complete)
...
def point_in_time_contract_issues(
    *,
    ...
    selectable = {
        ...
        for item in universe.get("tickers", [])  # type: ignore[arg-type]
        if str(item).strip()
    }
    if not selectable:
    ...
    timing_certified = certification.get("announcement_timing_certified") is True
    coverage_certified = certification.get("coverage_certified") is True
    if require_cash_announcement_timing and not timing_certified:
...
def build_frozen_candidate(
    *,
    ...
    """Freeze rank #1 before an untouched holdout; never choose using holdout data."""
    ...
    strategy = str(champion.get("trading_strategy", "")).strip().lower()
    management = str(champion.get("management_strategy", "")).strip()
    ...
    return {
        ...
        "frozen_at_utc": datetime.now().astimezone().isoformat(),
        "candidate": identity,
        ...
        "performance_claim_scope_if_validated": "single_untouched_historical_holdout",
...
def holdout_validation_issues(
    *,
    ...
    if str(realistic_summary.get("strategy", "")).strip().lower() != str(
        candidate.get("trading_strategy", "")
    ).strip().lower():
        issues.append("holdout_strategy_differs_from_frozen")
    if str(realistic_summary.get("management", "")).strip() != str(
        candidate.get("management_strategy", "")
    ).strip():
    ...
    if realistic_summary.get("selection_status") not in {
        "ex_ante_holdout",
        "prospective_frozen",
    }:
    ...
    if realistic_summary.get("economic_gap_adjustment") is True and pit_audit.get(
        "cash_announcement_timing_certified"
    ) is not True:
    ...
    *,
    ...
    return {
        ...
        "selection_classification": (
            ...
            if passed
            else "EX_ANTE_HOLDOUT_VALIDATION_BLOCKED"
        ),
        "candidate": frozen.get("candidate"),
        ...
        "data_snooping_control": "single_use_frozen_holdout_after_full_catalog_search",
