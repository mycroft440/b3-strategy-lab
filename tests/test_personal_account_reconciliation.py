...
class PersonalAccountReconciliationTests(unittest.TestCase):
            ...
            json.dumps(
                {
                    ...
                    "cash_balance": cash,
                    "positions": positions,
                    "source_document": f"broker-snapshot-{value_date}",
    ...
    def test_exact_reconciliation_requires_cash_and_positions_to_match(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._csv(
                ...
                [
                    {"trade_date": "2026-01-02", "settlement_date": "2026-01-06", "sequence": 10, "execution_id": "e1", "order_id": "o1", "side": "BUY", "ticker": "ABCD3", "shares": 10, "price": 10, "source_document": "note-1", "source_sha256": DIGEST},
                    {"trade_date": "2026-02-02", "settlement_date": "2026-02-04", "sequence": 10, "execution_id": "e2", "order_id": "o2", "side": "SELL", "ticker": "ABCD3", "shares": 4, "price": 12, "source_document": "note-2", "source_sha256": DIGEST},
                ],
            )
            self._csv(
                cash,
                ...
                [
                    {"value_date": "2026-01-06", "sequence": 20, "event_id": "f1", "kind": "B3_FEE", "amount": -0.10, "ticker": "ABCD3", "source_document": "note-1", "source_sha256": DIGEST},
                    {"value_date": "2026-02-04", "sequence": 20, "event_id": "f2", "kind": "B3_FEE", "amount": -0.05, "ticker": "ABCD3", "source_document": "note-2", "source_sha256": DIGEST},
                    {"value_date": "2026-03-01", "sequence": 10, "event_id": "d1", "kind": "DIVIDEND", "amount": 6.00, "ticker": "ABCD3", "source_document": "statement-1", "source_sha256": DIGEST},
                ],
            )
            self._csv(
                positions,
    ...
    def test_missing_fee_cent_blocks_exact_label(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._csv(
                fills,
                ["trade_date", "settlement_date", "sequence", "execution_id", "order_id", "side", "ticker", "shares", "price", "source_document", "source_sha256"],
                [{"trade_date": "2026-01-02", "settlement_date": "2026-01-06", "sequence": 0, "execution_id": "e1", "order_id": "o1", "side": "BUY", "ticker": "ABCD3", "shares": 10, "price": 10, "source_document": "note-1", "source_sha256": DIGEST}],
            )
    ...
    def test_duplicate_execution_id_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = [
                {"trade_date": "2026-01-02", "settlement_date": "2026-01-06", "sequence": 0, "execution_id": "same", "order_id": "o1", "side": "BUY", "ticker": "ABCD3", "shares": 1, "price": 10, "source_document": "note", "source_sha256": DIGEST},
                {"trade_date": "2026-01-02", "settlement_date": "2026-01-06", "sequence": 1, "execution_id": "same", "order_id": "o1", "side": "BUY", "ticker": "ABCD3", "shares": 1, "price": 10, "source_document": "note", "source_sha256": DIGEST},
            ]
    ...
    def test_invalid_source_hash_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "fills.csv"
            row = {"trade_date": "2026-01-02", "settlement_date": "2026-01-06", "sequence": 0, "execution_id": "e1", "order_id": "o1", "side": "BUY", "ticker": "ABCD3", "shares": 1, "price": 10, "source_document": "note", "source_sha256": "not-a-hash"}
            self._csv(path, list(row), [row])
            with self.assertRaisesRegex(ValueError, "source_sha256"):
                load_actual_fills(path)

    def test_ticker_change_can_be_reconciled_with_source_backed_position_events(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            opening = root / "opening.json"
            closing = root / "closing.json"
            ...
            self._csv(
                ...
                ["value_date", "sequence", "event_id", "ticker", "share_delta", "source_document", "source_sha256"],
                [
                    {"value_date": "2026-01-02", "sequence": 1, "event_id": "rename-out", "ticker": "OLD3", "share_delta": -5, "source_document": "broker-corporate-action", "source_sha256": DIGEST},
                    {"value_date": "2026-01-02", "sequence": 2, "event_id": "rename-in", "ticker": "NEW3", "share_delta": 5, "source_document": "broker-corporate-action", "source_sha256": DIGEST},
                ],
            )
            result = reconcile_actual_account(
                opening_snapshot=load_snapshot(opening),
