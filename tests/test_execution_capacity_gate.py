...
class ExecutionCapacityGateTests(unittest.TestCase):
    ...
    def test_generic_research_requires_capacity_gate(self) -> None:
    ...
    def test_certified_capacity_gate_rejects_oversized_sell_before_mutation(self) -> None:
        account = self._account()
        account.positions["AAA3"].shares = 200
    ...
    def test_splitting_an_order_does_not_reset_daily_capacity(self) -> None:
        account = self._account()
        account.buy_leg("2026-01-05", "AAA3", 60, self._quote())
        with self.assertRaisesRegex(ValueError, "remaining daily"):
            account.buy_leg("2026-01-05", "AAA3", 60, self._quote())
        self.assertEqual(account.shares("AAA3"), 60)
